...
def collocated_explicit(hold,x,dx,c,T,f):
    ...
    if f==True:
        ...
        for i in range(0,nt):#Iteration of the Collocated Explicit method over timesteps.
            ...
            for j in range(1,nx-1):#Iteration over the interior gridpoints to update h.
                if j==int(nx/2):
                    h[j]=hold[j]-0.5*c*(u[j+1]-u[j-1]) + np.sin(8*t[i])
                else:
    ...
    plt.plot(x, h, label='h(x,'+str(np.round(t[nt-1]))+')')
    plt.plot(x, u, label='u(x,'+str(np.round(t[nt-1]))+')')
    plt.title('Collocated Explicit method')
    plt.legend()
    plt.show()
